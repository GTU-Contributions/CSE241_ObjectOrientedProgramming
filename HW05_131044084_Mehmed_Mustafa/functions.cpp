    ...
    do
    {
        ...
        switch(choice)
        {
            case 1:
                ...
                if (test1 < test2)
                    cout << "First cell is smaller than second" << endl;
                else
                    cout << "First cell is bigger than second" << endl;
                break;
            case 2:
                cout << "First cell:" << endl;
                cin >> test1;
            ...
            case 3:
                ...
                cout << "Testing <= operator" << endl;
                if(test1 <= test2)
                    cout << "First cell is smaller or equal to second" << endl;
                else
                    cout << "First cell is bigger than second" << endl;
                break;
            case 4:
                ...
                cout << "Testing >= operator" << endl;
                if(test1 >= test2)
                    cout << "First cell is bigger or equal to second" << endl;
                else
                    cout << "First cell is smaller than second" << endl;
                break;
            case 5:
                ...
                cout << "Testing == operator" << endl;
                if(test1 == test2)
                    cout << "The choosen cells are equal" << endl;
                else
                    cout << "The choosen cells aren't equal" << endl;
                break;
            case 6:
                cout << "First cell:" << endl;
                cin >> test1;
        ...
        cout << "Do you want to test another operator ?" << endl;
        cout << "Press Y to test another option: ";
        cin >> checkEnd;
...
ostream& operator <<(ostream& output, const Cell &cell){
    output << "The X coordinate is: " << cell.getX() << endl;
    output << "The Y coordinate is: " << cell.getY() << endl;
    output << endl;
...
void Reversi::play(){
    ...
    gamePosition.push_back(gameCells);
...
//Operator overloading of the Reversi class
//Prefix version
...
//Prefix version
//Return the board after decrement
Reversi Reversi::operator --(){
    // If the gamePosition is the beggining do nothing
    if(gamePosition.size() == 1)
        return *this;
    else // Else take 1 move back
    {
        gameCells = gamePosition[gamePosition.size()-2];
        gamePosition.pop_back();
        return *this;
    }
}
//Postfix version
//Returns the board before decrement
Reversi Reversi::operator --(int ignoreMe){
    // If the gamePosition is the beggining do nothing
    if(gamePosition.size() == 1)
        return *this;
...
ostream& operator <<(ostream& output, const Reversi& game){
	...
	for(int column = 0; column<game.getWidth(); column++)
	{
		++columnNumber;
		if(columnNumber <= 26)
			output << setw(2) << game.intToString(columnNumber);
		else if(columnNumber <= 701)
			output << setw(3) << game.intToString(columnNumber);
	}
	output << endl; // Go to the next line
	...
	for(int row=0; row<game.getHeight(); ++row)
	{
		...
		for(int y=0; y<game.getWidth(); ++y)
		{
			...
			if(y <= 26)
			{
				bool isFind = false;
				//Check all cells in the given column
	for(unsigned int column = 0; column<game.gameCells[row].size(); ++column)
				{
					//if cell is found at Cell[x][y] print it's symbol
					if(y == game.gameCells[row][column].getY())
					{
						output << setw(2) << game.gameCells[row][column].getS();
						isFind = true;
					}
				}
				if(!isFind) // If the cell is not found, print blank
	...
	for(unsigned int column = 0; column<game.gameCells[row].size(); ++column)
				{
					...
					if(y == game.gameCells[row][column].getY())
					{
						output << setw(3) << game.gameCells[row][column].getS();
						isFind = true;
					}
				}
				if(!isFind) // If the cell is not found, print blank
...
Cell   Reversi::operator [](const string &move){
    int alphaPart;
    int numberPart;
    ...
    {
        alphaPart = toupper(move[0]+1) - 'A';
        numberPart = (move[1] -'0')*10 + (move[2] - '0');
    }
    
    char s; // junk
    if(isLivingCell(gameCells, --numberPart, --alphaPart, s))
        return Cell(++alphaPart, ++numberPart);
    else
        return Cell(-1000, -1000);
}
Cell   Reversi::operator ()(const string &move){
